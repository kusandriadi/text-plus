...
<div class="overlay" onclick={onClose} onkeydown={(e) => e.key === 'Escape' && onClose()} role="dialog" aria-label="Keyboard Shortcuts" tabindex="-1">
...
<style>
  ...
  .items.dark { background: #0e0e14; }
  .row {
    display: flex; justify-content: space-between; align-items: center;
    padding: 8px 12px;
  }
  .row.bordered { border-top: 1px solid #eee; }
  .row.bordered.dark { border-color: #2a2a35; }
  ...
  .kbd.dark { background: #0e0e14; border-color: #2a2a35; color: #999; }
